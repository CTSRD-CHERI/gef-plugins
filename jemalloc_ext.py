...
uaf_cmd_warn = False # Have we showed UAF command slow warning

class ScanCapsBase:
    """Base utility to scan memory for valid CHERI capabilities (tag=1)."""
    def __init__(self, should_print = None):
        self.should_print = should_print
        self.uaf_cmd_warn = False

    def get_readable_sections(self):
        return [
            section for section in gef.memory.maps
            if section.permission & Permission.READ and
            (section.page_end - section.page_start) < (1 << 40)
        ]

    def scan_memory_by_address(self, start_address, end_address, bounds = None):
        step = 0x400 * 0x1000
        locations = []
        streaks = [] # How many consecutive locations contain valid caps
        ...
        for chunk_addr in range(start_address, end_address, step):
            if chunk_addr + step > end_address:
                ...
                chunk_size = step
            try:
                bitmap = gef.memory.read_captags(chunk_addr, chunk_size)
                mem = gef.memory.read(chunk_addr, chunk_size)
            except (ValueError, gdb.error):
                err(f"Error at {chunk_addr:x} with size {chunk_size:x}")
                return []
            ...
            for b in bitmap:
                ...
                while b:
                    if b & 1:
                        cap_base = u64(mem[addr:addr + 8]) # Fast resolve
                        if not bounds:
                            locations.append((addr + start_address, cap_base))
                        if bounds:
                            for bound in bounds:
                                bound_start, bound_end = bound
                                if bound_start <= cap_base < bound_end:
                                    locations.append((addr + start_address, cap_base))
                                    break
                    b >>= 1
                    addr += 0x10 # Size of capability
                addr = base + 0x10 * 8 # Each byte has 8 bits
        return locations
    ...
    def print_loc(self, loc, addr, sect, cap_str) -> None:
        line_color = ""
        if sect:
            if sect.path == "[stack]":
            ...
            elif sect.permission & Permission.READ and sect.permission & Permission.EXECUTE:
                line_color = gef.config["theme.address_code"]
        gef_print(f"""  {loc:#x} {RIGHT_ARROW} {Color.colorify(f"{addr:#016x}" + cap_str, line_color)} """)
        return

    def scan_memory(self, maps, bounds = None):
        if not self.uaf_cmd_warn:
            stream_buf = gef.ui.stream_buffer
            gef.ui.stream_buffer = None # Hack to print warning immediately
            warn("Scanning memory, please wait... (this message shown once)")
            gef.ui.stream_buffer = stream_buf
            self.uaf_cmd_warn = True
        ...
        for section in maps:
            ...
            for res in self.scan_memory_by_address(start, end, bounds):
                if self.should_print == None or self.should_print(res):
                    loc, addr = res
                    addr_loc_start = lookup_address(loc)
                    cap = gdb.Value(loc).cast(u).dereference()
                    ...
                    self.print_loc(loc, addr, lookup_address(addr).section, parse_cap(cap))
...
@register
class ScanCapsCommand(GenericCommand, ScanCapsBase):
    """Scan memory in a given source range/address for valid CHERI capabilities pointing to
    a given destination range/address."""

    _cmdline_ = "scancaps"
    _syntax_  = f"{_cmdline_} [-h] [source] [destination]"
    _aliases_ = ["scan",] # this overrides the original gef scan command

    def __init__(self) -> None:
        GenericCommand.__init__(self, complete=gdb.COMPLETE_NONE)
        ScanCapsBase.__init__(self)
        return
    ...
    def do_invoke(self, argv: List[str], **kwargs: Any) -> None:
        ...
        if "0x" in needle:
            if "-" in needle:
                ...
                start = next(parse_string_range(needle))
                needle_sections.append((start, start + 1))

        for sect in gef.memory.maps:
            # so if I type [heap], that should be the only match
            # not [heap block]
            exact_match = False
            sect_path = sect.path
            if len(sect.path) > 0 and \
                sect.path[0] == "[" and \
                sect.path[-1] == "]":
                exact_match = True
                sect_path = sect_path.strip("[]")
            if not found_haystack:
                if (exact_match and sect_path == haystack) or \
                    (not exact_match and haystack in sect_path):
                    haystack_sections.append((sect.page_start, sect.page_end))
            if needle != "" and \
                ((exact_match and sect_path == needle) or \
                (not exact_match and needle in sect_path)):
                needle_sections.append((sect.page_start, sect.page_end))

        if len(needle_sections) == 0:
            self.scan_memory(haystack_sections)
        else:
            self.scan_memory(haystack_sections, needle_sections)
